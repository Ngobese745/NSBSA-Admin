...
class SearchProvider with ChangeNotifier {
  ...
  String _query = '';
  String _currentContext = 'global'; // 'global', 'groups', 'vendors', 'loans', 'payments'
    ...
    try {
      ...
      if (searchAll || contextType == 'groups') {
        ...
        if (baseQuery.isNotEmpty) {
          q = q.or('name.ilike.%$baseQuery%,reference_number.ilike.%$baseQuery%');
        }
        final groupResponse = await q;
        _groupResults = (groupResponse as List).map((e) => GroupModel.fromJson(e)).toList();
      }

      if (searchAll || contextType == 'vendors') {
        var q = _supabase.from('vendors').select();
        if (baseQuery.isNotEmpty) {
          q = q.or('name.ilike.%$baseQuery%,phone.ilike.%$baseQuery%,id_number.ilike.%$baseQuery%,business_type.ilike.%$baseQuery%,reference_number.ilike.%$baseQuery%');
        }
        final vendorResponse = await q;
        _vendorResults = (vendorResponse as List).map((e) => VendorModel.fromJson(e)).toList();
      }
      ...
      if (searchAll || contextType == 'loans') {
        ...
        _loanResults = (loanResponse as List).map((e) => LoanModel.fromJson(e)).toList();
        ...
        if (baseQuery.isNotEmpty && statusFilter == null && amountFilter == null) {
          ...
          if (amt != null) {
            _loanResults = _loanResults.where((l) => l.amount == amt).toList();
             ...
             _loanResults = _loanResults.where((l) => l.status.toLowerCase().contains(baseQuery.toLowerCase())).toList();
          }
        }
      }
      ...
      if (searchAll || contextType == 'payments') {
        ...
        _paymentResults = (paymentResponse as List).map((e) => PaymentModel.fromJson(e)).toList();
      }
